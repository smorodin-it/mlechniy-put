...
class CustomUser(AbstractBaseUser, PermissionsMixin, BaseAbstractModel):
    ...
    user_type = md.IntegerField(_('role'), choices=ROLE_CHOICES, default=PARTICIPANT)

    uuid = md.UUIDField(default=uuid4(), editable=False, unique=True)

    profile = md.OneToOneField(UserProfile, on_delete=md.CASCADE, null=True, default=None)
    ...
    def __str__(self):
        if self.profile is not None:
            return f'{self.profile.get_short_name()} - {self.get_user_type()}'
        return f'{self.email} - {self.get_user_type()}'
